...
Defines the structure of input and output files for Module 10,
...
INPUT_FILE_SCHEMAS: Dict[str, Dict[str, Any]] = {
    "synthetic_results/": {
        "fields": {
        "metrics.csv": {"type": "file", "required": True},
        "statistics.json": {"type": "file", "required": True},
        }
    },
    "experiments/<task_id>/": {
        "fields": {
        "processed_results/metrics.json": {"type": "file", "required": False},
        "raw_results/results.json": {"type": "file", "required": False},
        }
    },
...
OUTPUT_FILE_SCHEMAS: Dict[str, Dict[str, Any]] = {
    "decision.json": {
        "fields": {
        "decision": {"type": "str", "required": True, "allowed": ['PASS_TO_FIGURE_TABLE', 'RETURN_TO_EXPERIMENT', 'RETURN_TO_EXPERIMENT_PLAN', 'RETURN_TO_METHOD', 'RETURN_TO_INNOVATION', 'HUMAN_REVIEW_REQUIRED']},
        "reasoning": {"type": "str", "required": True},
        "claims_passed": {"type": "List[str]", "required": True},
        "claims_failed": {"type": "List[str]", "required": True},
        "claims_inconclusive": {"type": "List[str]", "required": True},
        "target_module": {"type": "str", "required": False},
        }
    },
    "claim_evidence_mapping.md": {
        "fields": {
        "mappings": {"type": "List[Dict]", "required": True},
        "mappings[].claim_id": {"type": "str", "required": True},
        "mappings[].claim_statement": {"type": "str", "required": True},
        "mappings[].evidence": {"type": "str", "required": True},
        "mappings[].verdict": {"type": "str", "required": True, "allowed": ['pass', 'fail', 'inconclusive']},
        "mappings[].data_origin": {"type": "str", "required": True},
        }
    },
}
